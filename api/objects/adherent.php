...
class Adherent {
    ...
    function read(){
        ...
        // select all query
        $query = "SELECT * FROM adherent ORDER BY id_adherent ASC";
    ...
    function readOne(){
        ...
        $query = "SELECT * FROM adherent WHERE id_adherent = :id LIMIT 0,1";
        ...
        $this->id = $row['id_adherent'];
        $this->id_demandeur = $row['id_demandeur'];
        $this->id_ligue = $row['id_ligue'];
        return true;
        }
       
    }    
// create new adherent record
function create(){
    ...
    $this->id_demandeur=htmlspecialchars(strip_tags($this->id_demandeur));
    
 
    // bind the values
    $stmt->bindParam(':id_ligue', $this->id_ligue);
    $stmt->bindParam(':id_demandeur', $this->id_demandeur);
...
function delete(){
    ...
    // delete query
    $query = "DELETE FROM adherent WHERE id_adherent = ?";
...
public function readPaging($from_record_num, $records_per_page){
    ...
    // select query
    $query = "SELECT * FROM adherent ORDER BY nom ASC LIMIT ?,?";
    // prepare query statement
    $stmt = $this->conn->prepare( $query );
